...
class PuerresMap {
    ...
    enableEditMode() {
        ...
        this.markers.forEach(marker => {
            if (marker.dragging) {
                ...
                // Agregar evento dragend para guardar automáticamente
                marker.off('dragend');
                marker.on('dragend', () => {
                    const coords = marker.getLatLng();
                    console.log(`📍 Marcador movido a: ${coords.lat}, ${coords.lng}`);
    ...
    async updatePOI() {
        ...
        // Guardar cambios en localStorage directamente
        try {
            const allPOIs = this.markers.map(m => {
                const coords = m.getLatLng();
                return {
                    id: m.poiData.id,
                    coords: [coords.lat, coords.lng],
                    name: m.poiData.name,
                    description: m.poiData.description || '',
                    whatsapp: m.poiData.whatsapp || '',
                    icon: m.poiData.icon,
                    color: m.poiData.color,
                    type: m.poiData.type
                };
            });
            
            localStorage.setItem(this.storageKey, JSON.stringify(allPOIs));
            console.log('✅ POIs guardados en localStorage');
            
            this.showLocationMessage('success', 'Punto de interés actualizado exitosamente');
        } catch (error) {
            this.showLocationMessage('error', 'Error al guardar los cambios');
    ...
    async deletePOI() {
        ...
        if (confirm('¿Estás seguro de que quieres eliminar este punto de interés?')) {
            ...
            try {
                ...
                this.showLocationMessage('success', 'Punto de interés eliminado exitosamente');
            } catch (error) {
                this.showLocationMessage('error', 'Error al guardar los cambios');
                console.error('❌ Error guardando en localStorage:', error);
            }
    ...
    async handleMapClickForAdd(e) {
        ...
        // Agregar marcador al mapa directamente
        const marker = this.addMarkerToMap(newPOI);
        
        if (marker) {
            console.log('✅ Marcador agregado exitosamente al mapa');
            ...
            try {
                ...
                this.showLocationMessage('success', 'Punto de interés agregado exitosamente');
                
                // Limpiar formulario
                this.cancelAddingPOI();
            } catch (error) {
                console.error('❌ Error guardando en localStorage:', error);
                this.showLocationMessage('error', 'Error al guardar el POI');
            }
        } else {
            console.error('❌ Error al agregar marcador al mapa');
            this.showLocationMessage('error', 'Error al agregar el marcador');
        }
    }
    ...
    // ==========================================
    // Cargar POIs desde localStorage
    // ==========================================
    loadSavedPOIs() {
        try {
            console.log('🔄 Cargando POIs desde localStorage...');
            const savedPOIs = localStorage.getItem(this.storageKey);
            
            if (savedPOIs) {
                const pois = JSON.parse(savedPOIs);
                console.log(`✅ ${pois.length} POIs encontrados en localStorage`);
                
                pois.forEach((poiData, index) => {
                    // Validar datos básicos
                    if (!poiData.name || !poiData.coords || !Array.isArray(poiData.coords)) {
                        console.error(`❌ POI ${index + 1} tiene datos inválidos, saltando...`);
                        return;
                    }
                    ...
                    if (marker) {
                        console.log(`✅ POI cargado: ${poiData.name}`);
                    }
                });
                
                console.log(`🎯 Total: ${this.markers.length} marcadores cargados`);
                return pois;
            } else {
                console.log('📭 No hay POIs guardados, cargando originales');
    ...
    savePOIsToStorage() {
        try {
            console.log('💾 Guardando POIs en localStorage...');
            ...
            // Preparar todos los POIs con sus posiciones actuales
            const allPOIs = this.markers.map(m => {
                const coords = m.getLatLng();
                return {
                    id: m.poiData.id,
            ...
            localStorage.setItem(this.storageKey, JSON.stringify(allPOIs));
            console.log(`✅ ${allPOIs.length} POIs guardados en localStorage`);
